...
describe('ChatInput', () => {
  it('renders input and action buttons', () => {
    ...
    expect(screen.getByRole('button', { name: /înregistrează mesaj vocal/i })).toBeInTheDocument();
  ...
  it('calls onSend with text content and clears input', () => {
    ...
    expect(onSend).toHaveBeenCalledWith({ type: 'text', text: 'Bună ziua' });
  ...
  it('shows closed message when conversationClosed is true', () => {
    ...
    expect(screen.getByText(/conversația este închisă/i)).toBeInTheDocument();
  });
